...
async def extract_task(page):
    ...
    # Try Explanation heuristic for SCRIPTED
    match = re.search(r'Explanation:[\s]+([^\n]+)', innerText)
    if match:
        return ("SCRIPTED", match.group(1).strip())
    
    # Try SCRIPTED heuristic
    match = re.search(r'SCRIPTED.*?exactly[\s]+([^\n]+)', innerText, re.IGNORECASE)
    if match:
        return ("SCRIPTED", match.group(1).strip())
...
async def main():
    ...
    async with async_playwright() as p:
        ...
        while True:
            # 1. Grid Page Detection: If we are on the grid page, dive into the first available Practice module
            ...
            if await practice_buttons.count() > 0:
                ...
                if await first_available.is_visible():
                    ...
                    task_num = 1  # Reset internal question counter
                    await page.wait_for_timeout(5000)
                    continue
            ...
            if not payload:
                ...
                clicked = await click_visible_button(page, [
                    ...
                    "a:has-text('Continue')", "a:has-text('Finish')",
                    "text=/Next|Continue|Submit|Finish/i"
                ], timeout=2000)
                ...
                try:
                    if ai_provider == 'gemini':
                        ...
                        response = sambanova_client.chat.completions.create(
                            ...
                            messages=[
                                {"role": "system", "content": "You are a confident, eloquent business professional taking an extempore speaking test."},
                                {"role": "user", "content": f"Write a natural, conversational 1-2 minute speech addressing this topic. Speak engagingly, as if giving a real response to the interviewer. Aim for roughly 150-250 words total.\n\nTopic Prompt:\n{payload}\n\nCRITICAL: Do not include ANY asterisks, markdown, placeholders, or stage directions. Just write the raw spoken words exactly as they should be dictated out loud."}
                            ],
                            temperature=0.7,
                            top_p=0.9
                        )
            ...
            try:
                ...
                try:
                    ...
                    # The stop button on this site is uniquely identified by the 'stop-button' class on a span element
                    for attempt in range(5):
                        try:
                            # Prefer exactly span.stop-button or anything with stop-button class
                            stop_btn = page.locator(".stop-button").first
                            if await stop_btn.is_visible(timeout=2000):
                                # Force to bypass any overlapping invisible layers
                    ...
                    # Always save HTML for debugging exactly after we theoretically trigger the stop
                ...
                if await check_btn.is_visible(timeout=10000):
                    # We give the click a massive 90-second timeout to wait for the button to become enabled
                ...
                try:
                    # Dynamically wait up to 45 seconds for the result (Success button or Error text)
                ...
                await page.wait_for_timeout(1000) # Small pad for DOM to settle
                ...
                clicked_next = await click_visible_button(page, [
                    "button:has-text('Submit')", "button:has-text('Next')", 
                    "button:has-text('Continue')", "button:has-text('Finish')",
                    "a:has-text('Submit')", "a:has-text('Next')", 
                    "a:has-text('Continue')", "a:has-text('Finish')",
